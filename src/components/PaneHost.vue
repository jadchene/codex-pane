...
<template>
  <PaneView
    ...
    @choose-image="store.chooseImage(pane)"
    @choose-files="store.chooseFiles(pane)"
    @open-skills="store.refreshSkills(pane)"
    @paste-image="store.addClipboardImage(pane)"
    @paste-reference="store.addPastedReference(pane, $event)"
