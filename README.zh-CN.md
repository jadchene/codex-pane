...
- 支持单窗格、左右双栏、上下双栏、四宫格、横向四栏、纵向四栏和固定 2×3 六宫格。
...
| 附件 | 点击一次附件按钮，在同一个文件选择器中选择图片或普通文件。 |
| 粘贴 | 文本直接进入输入框；剪贴板中的图片或文件保存为附件。 |
| 切换窗格 | 按 Alt+方向键。 |
| 滚动到底部 | 输入框为空时按方向键下。 |

选中的 Skill 会以 `@Skill名称` 保留在输入框中，并作为结构化 Skill 输入发送。图片和普通文件都会先复制到程序旁的 `data` 目录，再分别作为本地图片或文件引用发送。

每个窗格最多添加 20 个附件。本地图片最大 15 MB，单边不超过 8192 像素，总像素不超过 4000 万；普通文件最大 100 MB。
...
| PowerShell 7 路径 | 识别并移除命令显示中的 `pwsh -Command` 外层包装。 |
...
- 本地图片经过校验后复制到 `data/media`，普通文件复制到 `data/files`；app-server 只接收受管副本的路径。
